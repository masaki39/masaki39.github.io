...
const LINES: Line[] = [
  ...
  {
    type: "output",
    text: "masaki39の個人サイト。砂場遊びが大好きな息子(当時2歳)に由来する名前。Obsidianで書いたノートの一部を公開しています。",
  },
  { type: "empty", text: "" },
]
...
document.addEventListener("nav", () => {
  ...
  const toggleBtn = () => backToTop.classList.toggle("visible", window.scrollY > 300)
  ...
  // ── Terminal typing animation (index page only) ───────────────
  ...
  const typeNext = () => {
    ...
    if (charIdx < line.text.length) {
      ...
      lineIdx++
      charIdx = 0
      const pause = line.type === "command" ? LINE_PAUSE_AFTER_COMMAND : LINE_PAUSE_AFTER_OUTPUT
